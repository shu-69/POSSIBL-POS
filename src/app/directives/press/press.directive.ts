...
export class PressDirective implements OnInit {
  ...
  onPress(event: any) {
    ...
    this.customHandlePressing(event.type, event);
  }

  private handlePressing(type: any, event: any) { // touchend or touchstart
    if (type == 'touchstart') {
  ...
  private customHandlePressing(type: any, event: any) { // touchend or touchstart
    if (type == 'touchstart') {
      this.pressTimeout = setTimeout(() => {
        this.isPressing = true;
        // this.press.emit('start');
